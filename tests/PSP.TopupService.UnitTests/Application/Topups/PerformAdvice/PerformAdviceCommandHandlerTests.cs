...
/// <summary>
/// Verifies <see cref="PerformAdviceCommandHandler"/>: success path (terminal
/// Completed), retry path (re-enqueue AdviceRequested with backoff), terminal
/// advice failure (AdviceFailed, no auto-reversal) and idempotency replay.
/// </summary>
[Trait("Category", "Unit")]
...
public class PerformAdviceCommandHandlerTests
{
    ...
    private readonly Mock<IBankAdviceClient> _bank = new();
    ...
    [Fact]
    public async Task Handle_BankSuccess_Should_Complete_Topup_And_Enqueue_TopupCompleted()
    {
        ...
        _bank.Setup(b => b.AdviceAsync(tx.Id, tx.BankReference!, tx.Amount, tx.CorrelationId, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new BankAdviceResult { AdviceReference = adviceRef });
    ...
    [Fact]
    public async Task Handle_BankFailure_Below_MaxRetries_Should_Schedule_Retry_Via_Outbox()
    {
        var tx = AdvicePendingTransaction();
        _repository.Setup(r => r.GetByIdForUpdateAsync(tx.Id, It.IsAny<CancellationToken>())).ReturnsAsync(tx);
        _bank.Setup(b => b.AdviceAsync(It.IsAny<Guid>(), It.IsAny<TransactionReference>(), It.IsAny<Money>(), It.IsAny<Guid>(), It.IsAny<CancellationToken>()))
        ...
        result.FinalStatus.Should().Be(nameof(TopupStatus.AdvicePending)); // still pending
        ...
        _outbox.Verify(o => o.EnqueueAdviceRequestedAsync(tx.Id, tx.BankReference!, tx.Amount, tx.CorrelationId, It.IsAny<DateTime?>(), 1, It.IsAny<CancellationToken>()), Times.Once);
    }

    [Fact]
    public async Task Handle_BankFailure_At_MaxRetries_Should_Flag_AdviceFailed_Without_AutoReversal()
    {
        var tx = AdvicePendingTransaction();
    ...
    public async Task Handle_Should_Replay_When_Aggregate_Already_Terminal()
    {
        ...
        _bank.Verify(b => b.AdviceAsync(It.IsAny<Guid>(), It.IsAny<TransactionReference>(), It.IsAny<Money>(), It.IsAny<Guid>(), It.IsAny<CancellationToken>()), Times.Never);
    }
    ...
    private PerformAdviceCommandHandler BuildHandler() =>
        new(_repository.Object, _bank.Object, _outbox.Object, Options.Create(_options), NullLogger<PerformAdviceCommandHandler>.Instance);
